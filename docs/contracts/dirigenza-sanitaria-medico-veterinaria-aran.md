# CCNL Area Sanità 2022-2024 — ARAN (Dirigenti Medici e Veterinari SSN)

| | |
|---|---|
| **CNEL code** | `S225` |
| **Sector** | Pubblica Amministrazione |
| **Tax sector** | pubblica-amministrazione |
| **Employer signatories** | — |
| **Union signatories** | — |
| **Layer 1** | ✅ |
| **Layer 2** | ✅ |

[← Contracts index](index.md)

## Contract data

Full contract parameters, salary tables and coverage notes — sourced directly
from the bundled JSON:

```json
--8<-- "src/ccnl_engine/contract/data/dirigenza-sanitaria-medico-veterinaria-aran.json"
```

## Usage example

```python
--8<-- "docs/examples/contracts/dirigenza-sanitaria-medico-veterinaria-aran.py"
```
