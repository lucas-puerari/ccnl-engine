...
class TestCountAffectedScenarios:
    """count_affected_scenarios returns a structured ImpactResult."""

    def test_empty_iterable_returns_zeros(self) -> None:
        """No scenarios -> all counts are 0."""
        result = count_affected_scenarios([], date(2022, 1, 1), date(2026, 1, 1))
        assert result == ImpactResult(affected=0, evaluated=0, failed=0)
    ...
    def test_unaffected_scenario_not_counted(self) -> None:
        """A scenario whose result is the same at both dates is not counted."""
        scenarios = [_scenario(as_of=date(2026, 1, 1))]
        result = count_affected_scenarios(scenarios, date(2026, 1, 1), date(2026, 4, 1))
