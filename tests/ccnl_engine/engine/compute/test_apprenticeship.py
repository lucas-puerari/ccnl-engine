...
from ccnl_engine.domain.apprenticeship import ApprenticeshipPeriod
...
from ccnl_engine.engine.compute.apprenticeship import _find_period_index
...
class TestComputeApprenticePercentage:
    ...
    def test_no_tracks_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match=r"coverage\.layer_2 is partial"):
            compute_payslip(ccnl, _RULES, _req(employment=Apprentice(months_elapsed=0)))
    ...
    def test_pct_exempt_allowance_paid_at_full_value(self) -> None:
        ...
        ccnl = CCNL.model_validate(data)
        r = compute_payslip(ccnl, _RULES, _req(employment=Apprentice(months_elapsed=0)))
